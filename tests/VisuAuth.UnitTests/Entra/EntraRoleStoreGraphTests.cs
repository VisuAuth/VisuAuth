...
/// <summary>
/// Graph-touching paths of <see cref="EntraRoleStore"/>, mocked via
/// <see cref="FakeGraphHandler"/>. Parallels
/// <see cref="EntraUserStoreGraphTests"/> in shape — same auth-free
/// HttpClient swap, same canned JSON pattern.
/// </summary>
/// <remarks>
/// The role store is a thin shell over Graph's app-role endpoints +
/// service-principal lookup; these tests pin the projection /
/// short-circuit behaviour, not the Graph SDK itself.
/// </remarks>
public sealed class EntraRoleStoreGraphTests
{
    ...
    {
        var handler = new FakeGraphHandler().SetupGet("/servicePrincipals", ServicePrincipalJson);
        var role = await BuildStore(handler).GetAsync("99999999-9999-9999-9999-999999999999");
        role.Should().BeNull();
    }
    ...
    public async Task AssignRoleAsync_PostsAppRoleAssignment_OnHappyPath()
    {
        ...
        // to fill the PrincipalId field on the POST body.
        var handler = new FakeGraphHandler()
            .SetupGet("/servicePrincipals", ServicePrincipalJson)
    ...
    public async Task AssignRoleAsync_NonGuidUserId_ReturnsFailureWithFormatHint()
    {
        ...
        result.Error.Should().Contain("GUID",
            "the store calls Guid.Parse on the user id — non-GUID input must produce a clean error");
    }
    ...
    {
        // No target service principal means there's nothing to assign
        // against — the store bails before it ever inspects roles.
        var handler = new FakeGraphHandler().SetupGet("/servicePrincipals", EmptyServicePrincipalJson);
    ...
    private static EntraRoleStore BuildStore(FakeGraphHandler handler)
    {
